...
export const getDocument =(page: number, limit: number)=> {
    return request({
        url: `api/files/health_document/my-elderly/?limit=${limit}&page=${page}`,
        method: 'GET',
    }).then(response => {
...
export const getUserElderlyHealthDocument = (page: number, limit: number) => {
  return request({
      url: `api/files/health_document/family-elderly-health-documents/?limit=${limit}&page=${page}`,
      method: 'GET',
  }).then(response => {
    if (response.status === 200) {
      return response.data;
...
export const getCaregiverElderlyHealthDocument = (elderlyId: number, page: number, limit: number) => {
  return request({
      url: `api/files/health_document/caregiver-elderly/${elderlyId}/?limit=${limit}&page=${page}`,
      method: 'GET',
  }).then(response => {
    if (response.status === 200) {
      return response.data;
