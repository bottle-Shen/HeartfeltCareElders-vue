...
export const getHealthData =()=> {
    return request({
        // url: `api/data/health_data/my-elderly/?limit=10`,//limit表示每页展示的数据条数,这里设置为10
        url: `api/data/health_data/my-elderly/?limit=10`,//limit表示每页展示的数据条数,这里设置为10
        method: 'GET',
    }).then(response => {
...
//   // 确保 URL 以斜杠结尾
//   const sseUrl = `${baseURL}api/data/sse/`; // 拼接 SSE URL
//   return sseUrl;
// }
// 删除当前用户关联的老人的健康数据(只有机构人员有权限)
export const deleteHealthData = (healthDataId: number) => {
    return request({
        url: `api/data/health_data/delete-health-data/${healthDataId}/`,
        method: 'DELETE',
    }).then(response => {
      if (response.status === 200) {
        console.log(response.data)
...
export const getElderlyList = () => {
    return request({
        url: `api/data/health_data/elderly-list/`,
        method: 'GET',
    }).then(response => {
      if (response.status === 200) {
        // console.log(response.data)
...
export const getElderlyHealthDataList = (elderlyId: number) => {
    return request({
        url: `api/data/health_data/caregiver-elderly/${elderlyId}`,
        method: 'GET',
    }).then(response => {
      if (response.status === 200) {
        // console.log(response.data)
...
export const updateHealthData = (healthDataId: number,data: HealthDataFormType) => {
    return request({
        url: `api/data/health_data/update-health-data/${healthDataId}`,
        method: 'PATCH',
        data,
    }).then(response => {
      if (response.status === 200) {
...
export const getElderlyHealthData = () => {
  return request({
    url: `api/data/health_data/family-elderly-health-data/`,
    method: 'GET',
  }).then(response => {
    if (response.status === 200) {
      // console.log(response.data)
...
export const uploadHealthData = (elderlyId: number,data: HealthDataFormType) => {
  return request({
    url: `api/data/health_data/upload-health-data/${elderlyId}/`,
    method: 'POST',
    data,
  }).then(response => {
