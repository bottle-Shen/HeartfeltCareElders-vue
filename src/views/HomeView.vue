...
const HealthDataLength = ref<number>(0);// 获取到的健康数据长度
...
const fetchHealthData = async () => {
  try {
    ...
    hasElderlyHealthData.value = false;
...
const updateBtn = async(healthDataId: number,HealthData:HealthData) => {// 机构人员更新健康数据
  try {
    ...
    if (index !== -1) {
      healthData.value[index] = HealthData;

      HealthDataLength.value = healthData.value.length - 1; // 更新长度
      extractHealthData(); // 重新提取数据
      updateCharts(); // 更新图表
      // 如果当前行是选中的行，更新 selectedHealthData.value
      if (selectedHealthData.value.id === healthDataId) {
        selectedHealthData.value = { ...healthData.value[index] };
      }
    }
  } catch (error) {
...
const showdialogVisible = ()=>{
  selectedHealthData.value = {}; // 初始化为空对象
  dialogVisible.value = true
}
const openUpdateDialog = (row: HealthData) => {
      // 从最新的 healthData.value 中获取当前行的数据
    const currentRow = healthData.value.find(item => item.id === row.id);
    if (currentRow) {
      selectedHealthData.value = { ...currentRow }; // 深拷贝当前行的数据
      delete selectedHealthData.value.elderly_name; // 删除不需要的字段
  }
      dialogVisible.value = true;
};
// 上传健康数据
...
const saveHealthData = async () => {
  try {
    // console.log('保存',selectedHealthData.value)
    if (selectedHealthData.value.id) {
      // 更新操作
...
const submitForm = () => {
  if (healthDataForm.value) {
    // 在验证之前去除字段中的空格
    const validKeys = ['height', 'weight', 'systolic_pressure', 'diastolic_pressure', 'body_fat', 'muscle_mass', 'visceral_fat', 'body_temperature', 'blood_oxygen_saturation', 'heart_rate', 'blood_cholesterol', 'blood_triglyceride', 'high_density_cholesterol', 'low_density_cholesterol', 'blood_glucose'] as const;
    validKeys.forEach(key => {
      if (selectedHealthData.value[key] !== undefined) {
        if (typeof selectedHealthData.value[key] === 'string') {
          // 如果是字符串，去除空格并尝试转换为数字
          const trimmedValue = (selectedHealthData.value[key] as string).trim();
          // 检查字符串是否包含非法字符（如空格）
        if (trimmedValue.includes(' ')) {
          ElMessage.error(`${key} 字段必须为数字，不能包含空格`);
        } else {
          // 如果是字符串，尝试转换为数字
          const numericValue = parseFloat(trimmedValue);
          if (!isNaN(numericValue)) {
            (selectedHealthData.value[key] as string | number) = numericValue;
          } else {
            ElMessage.error(`${key} 字段必须为数字`);
          }
        }
      }
        if (typeof selectedHealthData.value[key] === 'number') {
          // 如果已经是数字，保留小数点后一位
          const roundedValue = Math.round(selectedHealthData.value[key] * 10) / 10;
          (selectedHealthData.value[key] as number) = roundedValue;
        }
      }
    });

    healthDataForm.value.validate(async(valid: boolean) => {
      if (valid) {
        // 表单验证通过，调用保存函数
        await saveHealthData();
      } else {
        // 表单验证失败，提示用户
        ElMessage.error('表单信息不完整，请填写所有必填项');
      }
    });
  }
};
const resetFormValidation = () => {
  if (healthDataForm.value) {
    healthDataForm.value.clearValidate(); // 清除验证状态
  }
};

// 监听 dialogVisible 的变化
watch(dialogVisible, (newVal, oldVal) => {
  if (!newVal && oldVal) {//如果对话框从 true（打开状态）变为 false（关闭状态）则重置表单验证
    // 当对话框关闭时重置表单验证
    resetFormValidation();
  }
});
const isTableVisible = ref(false)// 表格的显示状态
...
<template>
  <div class="health-data-page">
    <div class="header">
      ...
      <div v-if="isAuthenticated && getUserType === 3" class="header-button">
        <!-- 弹窗 -->
  <el-dialog v-model="dialogVisible" title="健康数据">
      <el-form class="w-full"
      :model="selectedHealthData" 
      :rules="formRules2"
      ref="healthDataForm"
      >
        <el-form-item label="身高" prop="height">
          <el-input v-model="selectedHealthData.height" />
        </el-form-item>
        <el-form-item label="体重" prop="weight">
          <el-input v-model="selectedHealthData.weight" />
        </el-form-item>
        <el-form-item label="收缩压" prop="systolic_pressure">
          <el-input v-model="selectedHealthData.systolic_pressure" />
        </el-form-item>
        <el-form-item label="舒张压" prop="diastolic_pressure">
          <el-input v-model="selectedHealthData.diastolic_pressure" />
        </el-form-item>
        <el-form-item label="体脂率" prop="body_fat">
          <el-input v-model="selectedHealthData.body_fat" />
        </el-form-item>
        <el-form-item label="肌肉量" prop="muscle_mass">
          <el-input v-model="selectedHealthData.muscle_mass" />
        </el-form-item>
        <el-form-item label="内脏脂肪面积" prop="visceral_fat">
          <el-input v-model="selectedHealthData.visceral_fat" />
        </el-form-item>
        <el-form-item label="体温" prop="body_temperature">
          <el-input v-model="selectedHealthData.body_temperature" />
        </el-form-item>
        <el-form-item label="血氧饱和度" prop="blood_oxygen_saturation">
          <el-input v-model="selectedHealthData.blood_oxygen_saturation" />
        </el-form-item>
        <el-form-item label="心率" prop="heart_rate">
          <el-input v-model="selectedHealthData.heart_rate" />
        </el-form-item>
        <el-form-item label="总胆固醇" prop="blood_cholesterol">
          <el-input v-model="selectedHealthData.blood_cholesterol" />
        </el-form-item>
        <el-form-item label="甘油三酯" prop="blood_triglyceride">
          <el-input v-model="selectedHealthData.blood_triglyceride" />
        </el-form-item>
        <el-form-item label="高密度脂蛋白胆固醇" prop="high_density_cholesterol">
          <el-input v-model="selectedHealthData.high_density_cholesterol" />
        </el-form-item>
        <el-form-item label="低密度脂蛋白胆固醇" prop="low_density_cholesterol">
          <el-input v-model="selectedHealthData.low_density_cholesterol" />
        </el-form-item>
        <el-form-item label="空腹血糖" prop="blood_glucose">
          <el-input v-model="selectedHealthData.blood_glucose" />
        </el-form-item>
        <el-form-item label="测量时间" prop="measured_at">
          <el-date-picker v-model="selectedHealthData.measured_at" type="datetime" placeholder="请选择测量日期和时间" />
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
        <el-button class="primary-button"  @click="dialogVisible = false">取消</el-button>
        <el-button class="primary-button" @click="submitForm">保存</el-button>
        </span>
      </template>
    </el-dialog>
    <div v-if="hasElderly" class="elderly-select">
      ...
      {{ isTableVisible ? '隐藏' : '更新/删除' }}
        ...
        <el-select v-model="selectedElderly" placeholder="请选择老人" @change="fetchHealthData">
          <el-option
  ...
  >
    ...
    <el-table-column prop="systolic_pressure" label="收缩压" show-overflow-tooltip />
      <el-table-column prop="diastolic_pressure" label="舒张压" show-overflow-tooltip />
      <el-table-column prop="body_fat" label="体脂率" show-overflow-tooltip />
      <el-table-column prop="muscle_mass" label="肌肉量" show-overflow-tooltip />
      <el-table-column prop="visceral_fat" label="内脏脂肪面积" show-overflow-tooltip />
      <el-table-column prop="body_temperature" label="体温" show-overflow-tooltip />
      <el-table-column prop="blood_oxygen_saturation" label="血氧饱和度" show-overflow-tooltip />
      <el-table-column prop="heart_rate" label="心率" show-overflow-tooltip />
      <el-table-column prop="blood_cholesterol" label="总胆固醇" show-overflow-tooltip />
      <el-table-column prop="blood_triglyceride" label="甘油三酯" show-overflow-tooltip />
      <el-table-column prop="high_density_cholesterol" label="高密度脂蛋白胆固醇" show-overflow-tooltip />
      <el-table-column prop="low_density_cholesterol" label="低密度脂蛋白胆固醇" show-overflow-tooltip />
      <el-table-column prop="blood_glucose" label="空腹血糖" show-overflow-tooltip />
      <el-table-column label="测量时间" show-overflow-tooltip>
      <template #default="scope">
        <!-- 使用 formatDate 函数格式化时间 -->
        <span>{{ formatDate(scope.row.measured_at) }}</span>
      </template>
    </el-table-column>
      <el-table-column prop="health_status" label="健康状态" show-overflow-tooltip />
      ...
      <ul v-if="number === 1" key="1" class="healthdata-header">
        <li class="header-item">
          ...
          <span class="header-item-title">
            <b>{{ healthMetrics.height[HealthDataLength] }}</b>
            <i>cm</i>
          </span>
        </li>
        <li class="header-item">
          ...
          <span class="header-item-title">
            <b>{{ healthMetrics.weight[HealthDataLength] }}</b><i>kg</i></span>
        </li>
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <span class="header-item-title">
          <b>{{ healthMetrics.bodyFat[HealthDataLength]}}</b><i>%</i></span></li>
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <span class="header-item-title">
          <b>{{ healthMetrics.muscleMass[HealthDataLength] }}</b><i>kg</i></span>
        </li>
      </ul>
      <ul v-else-if="number === 2" key="2" class="healthdata-header">
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <span class="header-item-title">
          <b>{{ healthMetrics.bodyTemperature[HealthDataLength] }}</b><i>°C</i>
        </span></li>
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <span class="header-item-title">
          <b>{{ healthMetrics.heartRate[HealthDataLength] }}</b><i>BPM</i></span></li>
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <span class="header-item-title">
          <b>{{ healthMetrics.systolicPressure[HealthDataLength]}}/{{ healthMetrics.diastolicPressure[HealthDataLength] }}</b><i>mmHg</i></span></li>
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <span class="header-item-title">
          <b>{{ healthMetrics.bloodOxygen[HealthDataLength] }}</b><i>%</i></span></li>
      </ul>
      <ul v-else-if="number === 3" key="3" class="healthdata-header">
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <i>总胆固醇</i></span>
          <span class="header-item-title">
          <b>{{ healthMetrics.cholesterol[HealthDataLength] }}</b><i>mmol/L</i></span></li>
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <span class="header-item-title">
          <b>{{ healthMetrics.bloodTriglyceride[HealthDataLength] }}</b><i>mmol/L</i></span></li>
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <span class="header-item-title">
          <b>{{ healthMetrics.systolicPressure[HealthDataLength]}}</b><i>cm<sup>2</sup></i></span></li>
        <li class="header-item">
          <span class="header-item-icon">
          ...
          <span class="header-item-title">
          <b>{{ healthMetrics.visceralFat[HealthDataLength] }}</b><i>mmol/L</i></span></li>
      </ul>
      </transition>
